...
check(
  "concept totals agree across sections",
  data.bundles.reduce((s, b) => s + b.concepts, 0) === data.counts.concepts &&
    data.conceptTypes.reduce((s, t) => s + t.count, 0) === data.counts.concepts,
  "bundle table, type table, and counts must describe the same 53 concepts"
)
check(
  "every audit finding has an owner",
  data.findings.every((f) => f.owner && f.severity),
  "a finding is missing severity or owner"
)
check(
  "diverging YAML cases are a subset of those tested",
  data.counts.yamlCasesDiverging > 0 &&
    data.counts.yamlCasesDiverging < data.counts.yamlCasesTested
)
